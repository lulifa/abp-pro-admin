...
namespace RuiChen.AbpPro.Identity
{
    public class DefaultIdentitySessionChecker : IIdentitySessionChecker, ITransientDependency
    {
        ...
        public async virtual Task<bool> ValidateSessionAsync(ClaimsPrincipal claimsPrincipal, CancellationToken cancellationToken = default)
        {
            ...
            if (lastAccressedTime.HasValue && lastAccressedTime.Value < accressedTime.Subtract(SessionCheckOptions.KeepAccessTimeSpan))
            {
                // 更新缓存中的访问地址以及客户端Ip地址
            ...
            // 避免某些场景频繁去刷新持久化设施
            if (lastAccressedTime.HasValue && lastAccressedTime.Value < accressedTime.Subtract(SessionCheckOptions.SessionSyncTimeSpan))
            {
